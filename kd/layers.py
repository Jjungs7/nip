...
class BasisCustModule(CustModule):
    def __init__(self):
        ...
        if self.args.coef_act_fn=='none':
            ...
            raise Exception("Unexpected activation function for mean of coefficient \"coef_act_fn:{}\"".format(coef_act_fn))
...
class CustWordEmb(CustModule):
    ...
    def forward(self, review, **kwargs):
        ...
        return x
class BasisCustWordEmb(BasisCustModule):
    def __init__(self, args):
        self.args = args
        super().__init__()
        self.word_dim = args.word_dim
    ...
    def forward(self, x, **kwargs):
        x = self.word_em(x) # N, L, Dw
        N, L, Dw = x.shape
        alpha = self.alpha(**kwargs) # N, B
        transition_on_bases = torch.stack([
            W(x) for W in self.BasesList # N, L, Dw
        ], dim=-2) # N, L, B, Dw
        transition = torch.matmul(
            alpha.unsqueeze(dim=1).repeat(1, L, 1).unsqueeze(-2), # N, L, 1, B
            transition_on_bases, # N, L, B, Dw
        ).squeeze(-2) # N, L, Dw
        return x + torch.tanh(transition)
...
class BasisCustBiLSTM(CustModule):
    def __init__(self, args):
        self.args = args
        super().__init__()
        ...
        for name, num_meta in args.meta_units:
            setattr(self, "num_"+name, num_meta)
            setattr(self, name, nn.Embedding(num_meta, args.attribute_dim))
        ...
        self.P = nn.Sequential(
            nn.Linear(args.attribute_dim*len(args.meta_units), args.key_query_size), # From MetaData to Query 
            nn.Tanh(),
    ...
    def init_param(self):
        super().init_param()
        colorlog.critical("layers.py: BasisCustBiLSTM: init_param: customized initialization is required")
        exit()
        pass
    def forward(self, x, length, **kwargs):
...
class CustAttention(CustModule):
    def __init__(self, args):
        self.args = args
        super().__init__()
        for name, num_meta in args.meta_units:
            setattr(self, "num_"+name, num_meta)
            setattr(self, name, nn.Embedding(num_meta, args.attribute_dim))
        self.attention = LinearAttentionWithQuery(encoder_dim=args.state_size, query_dim=args.attribute_dim*len(args.meta_units))
    def forward(self, x, mask, **kwargs):
        return self.attention(
            ...
            mask=mask)[0]
class BasisCustAttention(CustModule):
    def __init__(self, args):
        self.args = args
        super().__init__()
        for name, num_meta in args.meta_units:
    ...
    def init_param(self):
        super().init_param()
        colorlog.critical("layers.py: BasisCustAttention: init_param: customized initialization is required")
        exit()
        pass
    def forward(self, x, mask, **kwargs):
        return self.attention(
...
class CustBias(CustModule):
    def __init__(self, args):
        super().__init__()
        for name, num_meta in args.meta_units:
            setattr(self, "num_"+name, num_meta)
            setattr(self, name, nn.Embedding(num_meta, args.state_size))
        self.Y = nn.Linear(args.state_size*len(args.meta_units), args.num_labels, bias=False)
    def forward(self, **kwargs):
        return self.Y(torch.cat([
            getattr(self, name)(idx)
            for name, idx in kwargs.items()], dim=1))
class BasisCustBias(CustModule):
    def __init__(self, args):
        super().__init__()
        for name, num_meta in args.meta_units:
            setattr(self, "num_"+name, num_meta)
        ...
        self.Y = nn.Linear(args.state_size, args.num_labels, bias=False)
    def forward(self, **kwargs):
        return self.Y(
    ...
    def init_param(self):
        super().init_param()
        colorlog.critical("layers.py: BasisCustBias: init_param: customized initialization is required")
        exit()
        pass