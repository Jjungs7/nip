...
class NSC(nn.Module):
	def __init__(self, args):
		...
		self.WordEmb = nn.Embedding(self.args.vocab_size, self.args.word_dim)
		self.BiLSTM = TextLSTM(
			input_size=self.args.word_dim, hidden_size=self.args.hidden_dim//2, 
			bidirectional=True, bias=True, num_layers=self.args.lstm_num_layers,
		...
		self.Classifier = nn.Linear(self.args.hidden_dim, 5)
